...
from ArchRobot import DRAGONS
from ArchRobot.modules.helper_funcs.telethn import IMMUNE_USERS, telethn
...
async def archbots_is_admin(chat_id: int):
    status = False
    archbots = await telethn.get_me()
    async for user in telethn.iter_participants(
        chat_id, filter=ChannelParticipantsAdmins
    ):
        if archbots.id == user.id:
            status = True
            break
    return status
